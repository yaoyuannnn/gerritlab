...
def load_config(remote, repo):
    ...
    host_netloc = urlsplit(host).netloc
    project_id = gitreview_config["project_id"]

    private_token = _get_private_token(host_netloc, git_config, gitreview_config)
...
def _get_private_token(host_netloc: str,
    ...
    3. (DEPRECATED) The .gitreview file
    ...
    private_token = git_credentials.instance(host_netloc).get_token()
